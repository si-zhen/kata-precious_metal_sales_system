 ...
 */
public class ReductionOverTwoThousand extends AbstractReductionStrategy {
	...
	public Result calRealPay(BigDecimal unitPrice, int amount) {
		...
		int times = MathUtil.roundDown(allTotal, Constants.TWOTHOUSAND);
		if(times > 0){
			discountAmount = Constants.THIRTY.multiply(new BigDecimal(times));
